{ lib, engine, ... }:
let
  inherit (engine) shadow resolve inherit' inheritAll;
in
{
  "resolve" = {
    test-shadow-inner-wins = {
      expr = shadow { a = 1; b = 2; } { a = 99; c = 3; };
    ...
    test-resolve-local-wins = {
      expr = resolve { local = "L"; imported = "I"; inherited = "P"; };
      expected = "L";
    };

    test-resolve-imported-wins-over-inherited = {
      expr = resolve { local = null; imported = "I"; inherited = "P"; };
      expected = "I";
    };

    test-resolve-inherited-fallback = {
      expr = resolve { local = null; imported = null; inherited = "P"; };
      expected = "P";
    };

    test-resolve-all-null = {
      expr = resolve { local = null; imported = null; inherited = null; };
      expected = null;
    };
    ...
    test-inherit-stops-at-first = let
      roots = engine.buildNodes {
    ...
    test-inheritAll-accumulates = let
      roots = engine.buildNodes {
        parentGraph = engine.overlays [
          (engine.edge "c" "b")
          (engine.edge "b" "a")
