...
let
  ...
  toUpper = s: lib.concatStrings (map toUpperChar (lib.stringToCharacters s));
  ...
  tokenize =
    ...
    let
      ...
      go =
        pos: acc: currentToken: inString:
        if pos >= len then
          if currentToken != "" then acc ++ [ currentToken ] else acc
        else
          let
  ...
  classifyToken =
    ...
    else if lib.hasPrefix "'" tok then
      {
        type = "string";
        value = builtins.substring 1 (builtins.stringLength tok - 2) tok;
      }
    else if builtins.match "[0-9]+" tok != null then
      {
        type = "number";
        value = lib.strings.toInt tok;
      }
    else if isKeyword tok then
      {
        type = "keyword";
        value = toUpper tok;
      }
  ...
  consume =
    ...
    in
    if t == null then
      throw "sql-parser: unexpected end of input, expected ${expected}"
    else if t.value == expected then
      advance tokens
    else
      throw "sql-parser: expected '${expected}', got '${t.value}'";
  ...
  parseColumnRef =
    ...
    in
    if t == null then
      throw "sql-parser: expected column reference"
      ...
      if next != null && next.type == "dot" then
        ...
        if col != null && (col.type == "ident" || col.type == "star") then
          ...
          throw "sql-parser: expected column name after '.'"
      else
        {
          result = {
            table = null;
      ...
      throw "sql-parser: expected column reference, got ${t.type}";
  ...
  parseFrom =
    ...
    in
    if t == null then
      throw "sql-parser: expected table name in FROM clause"
    else
      let
        rest1 = advance tokens;
        next = peek rest1;
        ...
        isAlias =
          next != null
          && next.type == "ident"
          && !(builtins.elem (toUpper next.value) [
            "JOIN"
            "LEFT"
            "WHERE"
            "ORDER"
    ...
    let
      go =
        ...
        if t == null then
          ...
          let
            j = parseJoin false (advance toks);
          in
          go (acc ++ [ j.result ]) j.rest
          ...
          if next != null && next.value == "JOIN" then
            let
              j = parseJoin true (advance rest1);
            in
            go (acc ++ [ j.result ]) j.rest
          else
            {
  ...
  parseWhere =
    ...
    let
      ...
      parsePrimary =
        ...
        in
        if t == null then
          throw "sql-parser: expected WHERE expression"
          ...
          in
          if op == null then
            throw "sql-parser: expected operator in WHERE clause"
          # IS NULL / IS NOT NULL
            ...
            if next != null && next.value == "NOT" then
              let
                rest2 = consume "NULL" (advance rest1);
              in
              {
                result = {
                  op = "IS NOT NULL";
          ...
          # IN (...)
            ...
            let
              ...
              parseList =
                ...
                in
                if item == null then
                  throw "sql-parser: unexpected end in IN list"
                else if item.type == "rparen" then
                  {
                    result = acc;
                    rest = advance tks;
                  }
                else if item.type == "comma" then
                  parseList acc (advance tks)
                else
                  let
                    val =
                      if item.type == "string" then
                        item.value
                      else if item.type == "number" then
                        item.value
                      else
                        item.value;
                  in
          ...
          # LIKE 'pattern'
            ...
            if pattern == null then
              throw "sql-parser: expected pattern after LIKE"
            else
              {
                result = {
                  op = "LIKE";
          ...
          # =, !=, <, >, <=, >=
            ...
            in
            if right == null then
              throw "sql-parser: expected value after operator"
            ...
            throw "sql-parser: unexpected operator '${op.value}' in WHERE clause";
        ...
        let
          ...
          go =
            ...
            if t != null && t.value == "AND" then
              let
                right = parsePrimary (advance tks);
              in
              go {
                op = "AND";
                left = acc;
  ...
  parseLimit =
    ...
    if t != null && t.value == "LIMIT" then
      ...
      if num != null && num.type == "number" then
        ...
        throw "sql-parser: expected number after LIMIT"
    else
      {
        result = null;
        rest = tokens;
  ...
  parseSql =
    ...
    let
      ...
      whereResult =
        if hasWhere then
          parseWhere (advance whereTokens)
        else
          {
            result = null;
            rest = whereTokens;
