...
{ engine, lib }:
{
  ...
  overriddenKeys = self: id:
    ...
    in builtins.filter (key: builtins.length (allResults key) > 1) localKeys;

  configSources = self: id:
    let resolved = self.evaluated.${id}.get "resolvedConfig";
    in lib.mapAttrs (key: _:
      ...
      in if isLocal then "local" else if isImported then "import" else "inherited"
    ) resolved;
}
