...
let
  inherit (builtins) any filter isFunction length;
  ...
  ruleMatchesServer = fleet: rule: serverName:
    let
      whereClause = rule.where or null;
      matchFn = rule.match or null;
    in
    if matchFn != null then
      matchFn { inherit fleet serverName; server = fleet.server.${serverName}; }
      ...
      let
        ...
        trimmed = lib.trimWith { start = true; end = true; } whereClause;
        isFullQuery = lib.hasPrefix "SELECT" (lib.toUpper (lib.substring 0 6 trimmed));
        sqlQuery =
          if isFullQuery then trimmed
          else "SELECT name FROM servers WHERE ${whereClause}";
        results = queryFn fleet sqlQuery;
      in
      any (r: (r.name or r) == serverName) results;

  # Collect all rule modules that match a server
  matchingModules = fleet: rules: serverName:
    let
      matches = filter (r: ruleMatchesServer fleet r serverName) rules;
    in
    concatMap (r:
      let nixosCfg = r.nixos or {};
      in if isFunction nixosCfg
         then [ (nixosCfg { inherit fleet serverName; server = fleet.server.${serverName}; }) ]
         else [ nixosCfg ]
    ) matches;
  ...
  buildHostConfig = fleet: rules: baseModuleFn: serverName:
    let
  ...
  buildAllHostConfigs = fleet: rules: baseModuleFn:
    mapAttrs (name: _: buildHostConfig fleet rules baseModuleFn name) (fleet.server or {});
in
{
  inherit ruleMatchesServer matchingModules buildHostConfig buildAllHostConfigs;
}
